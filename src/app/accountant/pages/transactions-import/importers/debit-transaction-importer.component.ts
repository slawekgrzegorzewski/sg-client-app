...
export class DebitTransactionImporterComponent implements OnInit {
  ...
  private expenseToCreate: Expense | null = null;

  ngOnInit(): void {
  }

  canBeImported() {
    return this.expenseToCreate !== null;
  }

  private createExpense() {
    this.expenseToCreate = null;
    ...
    for (let transaction of this.transactions) {
      ...
      if (account.id !== accountOfCreatingExpense.id
        ...
        || (otherAccount !== null && otherAccount.currency !== accountOfCreatingExpense.currency)) {
        return;
      }
    ...
    if (amount.gt(new Decimal(0))) {
      ...
      this.expenseToCreate.description = description;
      this.expenseToCreate.amount = amount.toNumber();
      this.expenseToCreate.currency = accountOfCreatingExpense.currency;
      this.expenseToCreate.expenseDate = expenseDate;
    }
  }
