...
import {ClientPayment, ClientPaymentDTO} from '../../model/accountant/client-payment';
...
export class ClientPaymentsService {
  ...
  currentDomainClientPayments(date: Date): Observable<ClientPayment[]> {
    ...
    return this.http.get<ClientPaymentDTO[]>(`${this.endpoint}/${dateString}`)
      .pipe(map((data: ClientPaymentDTO[]) => (data.map(d => new ClientPayment(d)))));
  }
